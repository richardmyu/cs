...
如果检测到支持高精度时钟，chromium 会选择 `QueryPerformanceCounter`（QPC，微秒级），否则使用 `TimeGetTime`（精度与机器相关，默认大概15.6ms），chromium 在默认情况下会设置成 4 ms，但如果使用电池，会重置成默认精度。`setTimtout` 和 `setInterval` 等 Javascript 时钟函数， `DOMTimer.cpp` 里面已经写死了最小值就是 4 ms，所以 4 ms - 15.6 ms 成了这两个函数的精度。<sub>[3]</sub>
