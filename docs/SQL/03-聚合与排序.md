...
用于汇总的函数称为**聚合函数**或者聚集函数。所谓聚合，就是将多行汇总为一行。实际上，所有的聚合函数都是这样，输入多行输出一行。
...
第1行SELECT product_type, product_id
*/
```
...
/*
错误:  字段 "product.product_name" 必须出现在 GROUP BY 子句中或者在聚合函
数中使用
第4行HAVING product_name = '圆珠笔';
*/
```
