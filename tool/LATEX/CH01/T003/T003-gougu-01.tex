...
% gougu.tex
% 勾股定理
\documentclass[UTF8]{ctexart} %中文
\usepackage{graphicx} %提供插图功能
\usepackage{float} %浮动处理宏
\usepackage{amsmath} %专门用于公式的引用
\usepackage{geometry} %设计页面尺寸
\geometry{a6paper,centering,scale=0.8}

\usepackage[format=hang,font=small,textfont=it]{caption} %改变图标标题格式
\usepackage[nottoc]{tocbibind} %增加目录项目 nottoc 取消在显示中显示目录本身
...
都较古希腊更早。后者已经明确道出勾股定理的一般形式。图 \ref{fig:xuantu} 是我国古代对勾股定理的一种证明 \cite{quanjing}。
...
可以用符号语言表述为：设直接三角形 ABC，其中 $\angle{C} =90 \degree$，则有
\end{thm}
